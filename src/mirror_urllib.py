...
NOTE:
-- this version uses the standard urllib2 library instead of Requests, since
   not all environments have Requests installed. 
...
def activate_endpoint(base_url, headers, endpoint_name):
  the_url = base_url + "/endpoint/%s" % urllib.quote_plus(endpoint_name)
  r = urllib2.Request(url=the_url, headers=headers)
  print r.get_full_url()
  try: response = urllib2.urlopen(r)
  ...
  if (not info.has_key("activated")) or not info["activated"] :
    ...
    r = urllib2.Request(url=base_url + "/endpoint/%s/autoactivate" % urllib.quote_plus(endpoint_name), 
      headers=headers, data=urllib.urlencode(""))
...
def get_submission_id(base_url, headers):
  r = urllib2.Request(url=base_url + "/submission_id", headers=headers)
  response = urllib2.urlopen(r).read()
  print "Requested transfer id and got %s" % response
...
def submit_transfer(base_url, headers, submission_id, sync_level, source_endpoint, 
  ...
  r = urllib2.Request(url=base_url + "/transfer", headers=headers, data=payload)
  try: result = json.loads(urllib2.urlopen(r).read())
  except urllib2.HTTPError as e:
    print e.reason
